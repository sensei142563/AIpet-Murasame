...
【四、QQ 聊天怎么用】
  ...
  5) 主人专属指令（QQ 里发）：/clear 清空记忆、/switch 切换角色 等
     详见 设置 → QQ 配置 下方的说明
  6) ⚠ 本包自带的 NapCat 是**锁定版本**（见根目录 NAPCAT_VERSION.txt），
     **请不要更新它**：既不要在 NapCat 的 WebUI 页面里点「更新」，
     也不要自己下载新版覆盖 NapCat.Shell.Windows.OneKey 目录。
     新版 NapCat 与高版本 QQ 实测不稳定；遇到 QQ 发不出消息先看说明书
     「NapCat 打不开 / 发不出消息」一节，不要靠升级解决。
